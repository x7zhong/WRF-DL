...
ori_runner = OnnxEngine(model, 0)
ref_runner = OnnxEngine(model, 1)
print("ori provider = ", ori_runner.get_providers())
print("ref provider = ", ref_runner.get_providers())
import time
import pdb
pdb.set_trace()
shape1 = ori_runner.getInputShape()
diff = None
for i in [1, 10, 20, 40,50,  100,  150, 200 ,1000]:
    shape1[0] = 10000
    shape1[2] = 57
    ...
    for l in range (loop):
        ...
        try:
          ...
          print(f"inference exception:{inst}")
